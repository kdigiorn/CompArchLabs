			...
			;		r0 --> xzr
			;		r1 --> temp holding value
			;		r2 --> temp holding value
			...
			;		r11 --> (in swap) ptr to node 1, (in delete) ptr to node N
...
out4
			...
			cmp		r4, #4
			bne		hptr
			ldr		r5, [r5, #8]		; change the head pointer if swapping the first ele
hptr
			bl		swap
...
out2
			...
			;#################################################################################
			;		Iterate over list to look for duplicates (and delete them)
			;#################################################################################
			
			mov		r8, r0				; set i to 0
			mov		r10, r5			; r10 is iterating ptr (start at head)
loop5
			cmp		r8, r7
			bge		out5
			add		r8, r8, #4		; increment i
			ldr		r11, [r10]		; r11 is value of nth element in list
			ldr		r10, [r10, #8]	; iterate r10 to n+1 element in list
			ldr		r9, [r10]			; r9 now has value of n+1 element
loop6
			cmp		r11, r9			; compare r11 and r9 vals
			bne		out6				; if they're equal, delete n + 1 element
			ldr		r10, [r10, #4]	; r10 goes back to prev r10 ptr
			mov		r11, r10			; r11 is ptr to N (arg to delete function)
			bl		delete
			b		loop6
out6
			b		loop5
...
delete
			ldr		r12, [r11, #8]	; r11 is N
			ldr		r12, [r12, #8]	; r12 becomes N+2
			mov		r9, r11			; get addr of N's next ptr
			add		r9, r9, #8
			str		r12, [r9]			; load addr of r12 into N's next
			mov		r9, r12			; get addr of N+2's prev ptr
			add		r9, r9, #4
			str		r11, [r9]			; load addr of r11 into N+2's prev
			mov		r15, r14
